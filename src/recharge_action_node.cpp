...
public:
  ...
  : plansys2::ActionExecutorClient("recharge", 500ms)
  {
    ...
    detected_subscriber = this->create_subscription<std_msgs::msg::Bool>(
      "recharge/recharge_done", 10, std::bind(&RechargeAction::recharge_callback, this, std::placeholders::_1), sub_opt
    );
...
private:
  void do_work()
  {
    ...
    if ((old_fd_1!=fd_1)||(old_fd_2!=fd_2)) {
      ...
      RCLCPP_INFO_STREAM(this->get_logger(), "Sending FD 1: "<<fd_1);
  ...
  void recharge_callback(const std_msgs::msg::Bool::SharedPtr msg) {
    recharge_detected = msg->data;
    RCLCPP_INFO(this->get_logger(), "Recharge Done Received");
  }

  bool started;
  bool recharge_detected;
